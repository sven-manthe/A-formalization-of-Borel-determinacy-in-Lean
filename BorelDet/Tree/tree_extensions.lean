...
def pointedRes (k : ℕ) : PointedTrees ⥤ PointedTrees where
  ...
  map {S T} f := ⟨(forgetPoint ⋙ res k).map f, by
    ...
    rw [take_apply_val _ k]; simp [pointedResObj]⟩
  map_id _ := rfl
  map_comp _ _ := PointedLenHom.ext rfl
theorem pointedRes_isIso_iff_fixing k {S T : PointedTrees} (f : S ⟶ T) :
...
def extensionsRes T :
  ...
  left_inv _ := rfl
  right_inv _ := rfl
@[simp] theorem extensionsRes_val' {T : Trees} {x : T} (a : ExtensionsAt x) :
...
@[simp] theorem extensionsRes_symm_val' {T : Trees} {x : T} a :
  ExtensionsAt.val' (A := no_index _) ((extensionsRes (mkPointed x)).symm a)
  = extensions.val' a := by
  simp only [extensions.val', ExtensionsAt.val', pointedRes,
    le_add_iff_nonneg_right, zero_le, List.take_of_length_le]; rfl
@[simp] theorem cast_val' {S : PointedTrees} (h : k = m)
...
@[simp] theorem extensionsEquiv_val' (a : ExtensionsAt x) hx :
...
@[simp] theorem extensionsEquiv_symm_val'
...
@[simp] theorem extensionsEquiv'_symm_val'
...
@[simp] theorem extensionsEquiv'_val' (a : ExtensionsAt y) (hy : Fixing (y.val.length + 1) f) :
  (extensionsEquiv' f y hy a).valT' = pInv f a.valT' := by
  obtain ⟨a, rfl⟩ := (extensionsEquiv' f y).symm.surjective a
  simp only [Equiv.apply_symm_apply, extensionsEquiv'_symm_val', Subtype.coe_eta, cancel_pInv_left]

@[simp] lemma val_res_zero (x : (res 0).obj S) : x.val = [] :=
  List.eq_nil_of_length_eq_zero (Nat.le_zero.mp (res_len_le x))
theorem zero_fixing : Fixing 0 f ↔ ([] ∈ S.2 ↔ [] ∈ T.2) := by
...
theorem lim_isPruned (F : ℕᵒᵖ ⥤ Trees)
  ...
  apply h
theorem lim_ne (F : ℕᵒᵖ ⥤ Trees) (hF : ∀ n, Tree.Fixing n (F.map (homOfLE (Nat.le_succ n)).op))
